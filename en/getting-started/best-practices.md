...
>
> **TezSign** is now the recommended signing solution for new Tezos bakers. It offers better performance and is purpose-built for baking.
>
> **Ledger** (Nano S Plus or Nano X) remains a valid option, especially for existing setups. The original Ledger Nano S is no longer recommended due to slower signing times that can cause missed attestations.

### Two computers

You will need two computers and both computers will be running TezBake. One computer will be used for baking and the other computer will be used as a backup. A manual failover of the hardware signer will be necessary to start baking on the second computer if the first computer fails. The second computer is optional, but it is highly recommended. If you only have one computer, you will need to have a backup plan in place in case the computer fails.

### Backup Hardware Signer

While having two computers is somewhat optional, especially for a smaller baker, having a backup hardware signer (TezSign or Ledger) is very highly recommended. You will need a backup for the following reasons:

* In the event of hardware signer failure
